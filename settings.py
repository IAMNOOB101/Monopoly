...
class GameMechanics:
    # Houses and hotel available for development (official rules: 32 houses, 12 hotels)
    available_houses = 32
    available_hotels = 12
    ...
    # Auction settings (official rules: unowned property must be auctioned if buyer declines)
    enable_auctions = True
    auction_min_bid = 1  # Minimum opening bid
    
    # Bankruptcy transfer rules
    # Official rule: new owner of mortgaged property must pay 10% tax immediately
    bankrupt_mortgage_tax = True
...
class StandardPlayerSettings:
    ...
    # Monopoly blocking: refuse trades that grant opponent a high-value monopoly
    enable_monopoly_blocking: bool = True
    # Housing shortage strategy: skip hotel upgrades on cheap groups to exhaust house supply
    enable_housing_shortage_strategy: bool = False
    # Dynamic cash buffer: scale unspendable_cash based on board danger
    dynamic_cash_buffer: bool = False
