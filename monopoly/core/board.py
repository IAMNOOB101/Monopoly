...
class Board:
    ...
    def run_auction(self, property_cell, players, dice, log):
        """ Run a sealed-bid auction for a property among all non-bankrupt players.

        Each AI player calculates their maximum willingness to pay.
        The highest bidder wins at their bid price.
        If no one bids, the property remains unowned.

        Official rules: when a player declines to buy, the bank auctions
        the property to ALL players (including the one who declined).
        """
        active_bidders = [p for p in players if not p.is_bankrupt]
        ...
        for player in active_bidders:
            max_bid = player.get_auction_bid(property_cell, self)
            if max_bid >= GameMechanics.auction_min_bid:
                bids.append((player, max_bid))

        if not bids:
            log.add(f"Auction for {property_cell}: No bids. Property remains unowned.")
            return
        ...
        # If there's a tie, the first player in turn order wins (already sorted by players list order)

        # Winner pays and receives property
        winner.money -= winning_bid
        property_cell.owner = winner
        winner.owned.append(property_cell)

        log.add(f"Auction: {winner} wins {property_cell} for ${winning_bid}")

        # Recalculate monopolies
        self.recalculate_monopoly_multipliers(property_cell)

        # Recalculate trade lists for all players
        for player in players:
            player.update_lists_of_properties_to_trade(self)
