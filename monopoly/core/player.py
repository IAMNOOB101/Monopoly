...
from monopoly.core.constants import INDIGO, BROWN, RAILROADS, UTILITIES, ORANGE, RED, GREEN, YELLOW
from monopoly.core.move_result import MoveResult
from settings import GameMechanics

# High-value color groups used for monopoly blocking decisions
HIGH_VALUE_GROUPS = frozenset({ORANGE, RED, YELLOW, GREEN, INDIGO})

# Cheap color groups eligible for the housing shortage strategy
CHEAP_GROUPS = frozenset({BROWN, "Lightblue"})
...
class Player:
    ...
    def get_effective_cash_buffer(self, board):
        """Calculate the effective cash buffer based on board danger.

        If dynamic_cash_buffer is enabled, the buffer scales up when
        opponents have developed monopolies (high-rent danger zones)
        on the path ahead.

        Returns the effective unspendable_cash threshold.
        """
        base_buffer = self.settings.unspendable_cash

        if not self.settings.dynamic_cash_buffer:
            return base_buffer

        # Calculate board danger: sum of max rents from opponents' developed properties
        max_danger = 0
        for cell in board.cells:
            if not isinstance(cell, Property):
                continue
            if cell.owner is None or cell.owner == self:
                continue
            if cell.is_mortgaged:
                continue

            # Estimate max rent this property could charge
            if cell.has_hotel:
                rent = cell.rent_house[-1]
            elif cell.has_houses > 0:
                rent = cell.rent_house[cell.has_houses - 1]
            elif cell.monopoly_multiplier >= 2:
                rent = cell.rent_base * cell.monopoly_multiplier
            else:
                continue  # Undeveloped non-monopoly: low threat

            if rent > max_danger:
                max_danger = rent

        # Scale buffer: base + fraction of the max danger rent
        # Capped at 3x the base buffer to avoid hoarding
        danger_bonus = min(max_danger // 3, base_buffer * 2)
        return base_buffer + danger_bonus

    def get_auction_bid(self, property_cell, board):
        """Determine the maximum bid for a property at auction.

        Strategy:
        - Base bid: up to the property's face value
        - If buying completes a monopoly: bid up to 2x face value
        - If buying blocks an opponent's monopoly: bid up to 1.5x face value
        - Never bid more than available cash minus effective buffer
        """
        if self.is_bankrupt:
            return 0

        effective_buffer = self.get_effective_cash_buffer(board)
        available_cash = self.money - effective_buffer

        if available_cash <= 0:
            return 0

        # Property is in a group the player chose to ignore
        if property_cell.group in self.settings.ignore_property_groups:
            return 0

        base_value = property_cell.cost_base
        bid_multiplier = 1.0

        group_cells = board.groups.get(property_cell.group, [])

        # Check if buying this would complete OUR monopoly
        our_count = sum(1 for c in group_cells if c.owner == self)
        needed_for_monopoly = len(group_cells) - 1  # We'd own this one too

        if our_count == needed_for_monopoly:
            # Completing a monopoly — very valuable
            bid_multiplier = 2.0

        # Check if this blocks an opponent's monopoly
        for c in group_cells:
            if c.owner is not None and c.owner != self:
                opponent = c.owner
                opponent_count = sum(1 for cc in group_cells if cc.owner == opponent)
                if opponent_count == len(group_cells) - 1:
                    # Opponent needs only this one — block them
                    bid_multiplier = max(bid_multiplier, 1.5)

        max_bid = int(base_value * bid_multiplier)
        return min(max_bid, available_cash)
    ...
    def handle_community_chest(self, board, players, log):
        ...
        # BUG FIX: Was "You inherit $100""You inherit $100" (accidental string concatenation)
        ...
        # Receiving money from other players
            ...
            for other_player in players:
                if other_player != self and not other_player.is_bankrupt:
                    # BUG FIX: Was pay_money(50, ...) — should be $10 per the card text
                    other_player.pay_money(10, self, board, log)
    ...
    def handle_landing_on_property(self, board, players, dice, log):
        ...
        def is_willing_to_buy_property(property_to_buy):
            ...
            if self.money - property_to_buy.cost_base < effective_buffer:
                return False
        ...
        if landed_property.owner is None:
            ...
            if is_willing_to_buy_property(landed_property):
                ...
                # Official rule: Bank auctions the property to all players
                if GameMechanics.enable_auctions:
                    board.run_auction(landed_property, players, dice, log)
    ...
    def improve_properties(self, board, log):
        ...
        If enable_housing_shortage_strategy is on, skip hotel upgrades
        on cheap color groups to deplete the bank's house supply.
        """
        ...
        def get_next_property_to_improve():
            ...
            for cell in self.owned:
                ...
                if (
                    ...
                    # Housing shortage strategy: skip hotel upgrade on cheap groups
                    # to keep houses locked up and starve opponents
                    if (self.settings.enable_housing_shortage_strategy
                            and cell.has_houses == 4
                            and cell.group in CHEAP_GROUPS):
                        continue
        ...
        while True:
            ...
            effective_buffer = self.get_effective_cash_buffer(board)
            if self.money - improvement_cost < effective_buffer:
                break
    ...
    def unmortgage_a_property(self, board, log):
        ...
        for cell in self.owned:
            if cell.is_mortgaged:
                ...
                if self.money - cost_to_unmortgage >= effective_buffer:
    ...
    def pay_money(self, amount, payee, board, log):
        ...
        def transfer_all_properties(payee, board, log):
            ...
            Official rule: when inheriting mortgaged property, the new owner must
            immediately pay 10% of the mortgage value as a tax to the bank.
            """
            ...
            while self.owned:
                ...
                if isinstance(payee, Player):
                    ...
                    # Official rule: pay 10% tax on mortgaged properties received
                    if cell_to_transfer.is_mortgaged and GameMechanics.bankrupt_mortgage_tax:
                        tax = int(cell_to_transfer.cost_base * GameMechanics.mortgage_value
                                  * GameMechanics.mortgage_fee)
                        if payee.money >= tax:
                            payee.money -= tax
                            log.add(f"{payee} pays ${tax} mortgage tax on {cell_to_transfer}")
                        else:
                            # Can't pay tax: unmortgage cost will be owed later
                            log.add(f"{payee} cannot afford ${tax} mortgage tax on {cell_to_transfer}")
    ...
    def do_a_two_way_trade(self, players, board, log):
        ...
        def would_complete_opponent_monopoly(trade_gives, other_player):
            """Check if giving these properties would complete the opponent's monopoly
            on a high-value color group."""
            if not self.settings.enable_monopoly_blocking:
                return False

            for cell in trade_gives:
                if cell.group not in HIGH_VALUE_GROUPS:
                    continue
                group_cells = board.groups.get(cell.group, [])
                # Count how many of this group the opponent would own after the trade
                opponent_count = sum(1 for c in group_cells if c.owner == other_player)
                # +1 for the cell being given
                if opponent_count + 1 >= len(group_cells):
                    return True
            return False

        def would_complete_our_monopoly(trade_receives):
            """Check if receiving these properties would complete our monopoly."""
            for cell in trade_receives:
                group_cells = board.groups.get(cell.group, [])
                our_count = sum(1 for c in group_cells if c.owner == self)
                if our_count + 1 >= len(group_cells):
                    return True
            return False
        ...
        for other_player in players:
            ...
            if self.wants_to_buy.intersection(other_player.wants_to_sell) and \
                ...
                # Monopoly blocking: refuse to trade if it completes opponent's high-value monopoly
                # unless we also complete a monopoly of our own
                if would_complete_opponent_monopoly(player_gives, other_player):
                    if not would_complete_our_monopoly(player_receives):
                        log.add(f"{self} blocks trade with {other_player} "
                                f"(would complete opponent's monopoly)")
                        continue
