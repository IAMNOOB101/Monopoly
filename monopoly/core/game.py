...
4. Tracking net worth and landing positions per turn
"""
...
def monopoly_game(game_number_and_seeds: Tuple[int,int]) -> None:
    ...
    board, dice, events_log, bankruptcies_log, networth_log, landings_log = setup_game(game_number, game_seed)
    ...
    for turn_n in range(1, SimulationSettings.n_moves + 1):
        ...
        # ── Per-Turn Tracking ───────────────────────────────
        # Record net worth and landing positions for all alive players
        for player in players:
            if not player.is_bankrupt:
                nw = player.net_worth()
                networth_log.add(f"{game_number}\t{turn_n}\t{player.name}\t{nw}")
                landings_log.add(f"{game_number}\t{turn_n}\t{player.name}\t{player.position}")
    ...
    if networth_log.content:
        networth_log.save()
    if landings_log.content:
        landings_log.save()
...
def setup_game(game_number, game_seed):
    ...
    # Net worth tracking log (per-turn, per-player)
    networth_log = Log(LogSettings.NETWORTH_LOG_PATH)

    # Landing position tracking log (per-turn, per-player)
    landings_log = Log(LogSettings.LANDINGS_LOG_PATH)
    ...
    return board, dice, events_log, bankruptcies_log, networth_log, landings_log
